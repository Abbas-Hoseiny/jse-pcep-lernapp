/* =========================================================
   PCEP Lern-App — Shared Logic (Python via Pyodide)
   ========================================================= */
(function () {
  "use strict";

  // ---- Progress ------------------------------------------
  const STORAGE_KEY = "pcep_progress_v1";
  const defaultProgress = { modules: {}, examBest: null, examAttempts: [] };
  ...
  window.PCEP_PROGRESS = { load: loadProgress, save: saveProgress, update: updateModule };
  ...
  const PY_BUILTINS = [
    "print","input","int","float","str","bool","list","tuple","dict","set","frozenset",
    "len","range","type","isinstance","id","abs","min","max","sum","sorted","reversed",
  ...
  function initFlashcards({ container, cards, moduleId }) {
    ...
    hint.textContent = "Klick zum Umdrehen · Leertaste: umdrehen · ← → : navigieren";
    ...
    function markKnown() { knownSet.add(order[idx]); next(); }
    function again() {
      const cur = order[idx];
      order.splice(idx, 1);
      order.push(cur);
    ...
    function reset() { order = shuffle([...cards.keys()]); idx = 0; knownSet = new Set(); render(); }
  ...
  function ensurePyodide(onStatus) {
    ...
    if (typeof window.loadPyodide !== "function") {
      // Dynamically add the Pyodide script
      pyodidePromise = new Promise((resolve, reject) => {
        onStatus && onStatus("Lade Python-Interpreter (~10 MB, einmalig)...");
        const s = document.createElement("script");
        s.src = "https://cdn.jsdelivr.net/pyodide/v0.27.0/full/pyodide.js";
        s.onload = async () => {
          try {
            onStatus && onStatus("Initialisiere Python...");
            pyodide = await window.loadPyodide({
              indexURL: "https://cdn.jsdelivr.net/pyodide/v0.27.0/full/"
            });
            onStatus && onStatus("Bereit.");
            resolve(pyodide);
          } catch (err) { reject(err); }
        };
        s.onerror = () => reject(new Error("Pyodide konnte nicht geladen werden (Internet?)"));
  ...
  function initPlaygrounds(playgrounds) {
    playgrounds.forEach(pg => {
      ...
      async function run() {
        ...
        loader.innerHTML = '<div class="spinner"></div><span class="js-status">Starte...</span>';
        out.appendChild(loader);
        const setStatus = (msg) => { loader.querySelector(".js-status").textContent = msg; };
        ...
        try {
          ...
          if (stdoutBuf) {
            ...
            d.textContent = stdoutBuf;
            out.appendChild(d);
          }
          if (stderrBuf) {
            const d = document.createElement("div");
            d.className = "err";
            d.textContent = stderrBuf;
            out.appendChild(d);
          }
          if (!stdoutBuf && !stderrBuf) {
            out.textContent = "# (keine Ausgabe)";
          }
        } catch (err) {
          out.innerHTML = `<div class="err">Fehler beim Laden von Python: ${escapeHTML(String(err.message || err))}</div>`;
        }
      }
  ...
  // ---- Exam engine (PCEP: 30 Fragen, 45 Min, 70%) --------
  function initExam({ container, pool, count = 30, minutes = 45, passPct = 70 }) {
    if (!container || !pool || !pool.length) return;
    const root = document.createElement("div");
    root.innerHTML = `
      <div class="exam-intro theory-section">
        <h2 style="margin-top:0">Prüfungssimulation — PCEP-30-02</h2>
    ...
    let currentIdx = 0, timerId = null, remainingSec = minutes * 60;
    ...
    function renderExam() {
      ...
      updateTimer(); renderNav(); renderQuestion();
  ...
  function renderDashboard() {
    ...
    document.querySelectorAll("[data-module-progress]").forEach(el => {
      ...
      const pct = Math.round(((d + fcPct) / 3) * 100);
  ...
  function injectSidebar() {
    ...
    const modules = [
      ...
      { id: "5", label: "Datenstrukturen" },
      { id: "6", label: "Funktionen" },
    ...
    sb.innerHTML = `
      <div class="sb-brand">
        <div class="sb-brand-logo">Py</div>
        <div class="sb-brand-text">
          <div class="t1">PCEP Lern-App</div>
          <div class="t2">Python Essentials — 30-02</div>
        </div>
      </div>
    ...
    sb.querySelector('[data-action="reset-progress"]').addEventListener("click", (e) => {
      e.preventDefault();
      if (confirm("PCEP-Fortschritt löschen?")) {
  ...
  window.PCEP = { initFlashcards, initQuiz, initPlaygrounds, initExam, highlight: highlightAll, renderDashboard };
})();
